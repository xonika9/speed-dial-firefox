...
import { getObjectUrl, mod, openFolder, run, urlToDomain } from './utils/assorted';
...
const Search: Component<SearchProps> = props => {
    const [nodes, setNodes] = createSignal<ReadonlyArray<Bookmarks.BookmarkTreeNode>>([]);
    ...
    async function loadNodes() {
        const ns = (await getSubTreeAsList(rootFolderId)).filter(n => !isSeparator(n));
        setNodes(ns);
    }
    ...
    async function loadFavicons(
        ...
        const items = await Promise.allSettled(
            nodes.filter(isBookmark).map(async (node): Promise<[string, Blob | undefined]> => {
