...
import { ContextMenu, ContextMenuItem, ContextMenuSeparator, ctxMenuIconSize } from './ContextMenu';
        ...
        <>
            <Show when={folderItem.item.unmodifiable == null}>
                ...
                <ContextMenuItem
                    ...
                    onClick={async () => {
                        children = (await getSubTreeAsList(folderItem.item.id)).filter(isBookmark);
                ...
                >
                    ...
                    <Modal show={showDeleteModal()} onClose={() => setShowDeleteModal(false)}>
                        <div class='modal-content' style={{ 'max-width': '550px' }}>
                            Confirm you would like to delete the folder "{folderItem.item.title}"
                            {children.length > 0
            ...
            >
                Open Children
                <Modal show={showChildrenModal()} onClose={() => setShowChildrenModal(false)}>
...
const FolderTile: Component = () => {
    ...
    const [children, setChildren] = createSignal<ReadonlyArray<Bookmarks.BookmarkTreeNode>>();
    ...
    bookmarks.getChildren(folderItem.item.id).then(children => setChildren(children.slice(0, 4)));
    ...
    return (
        <TileCard backgroundColor='rgba(var(--background-rgb), 0.5)' onContextMenu={setOnContext}>
