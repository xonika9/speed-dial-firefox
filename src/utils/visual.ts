...
export function isMemoBookmarkVisualMeta(
    ...
    return vis != null && vis !== 'loading' && vis !== 'folder' && vis !== 'separator';
}

export const bookmarkVisual = memoDefault(
    (
        id: string,
    ): [Accessor<MemoBookmarkVisual>, (data: MemoBookmarkVisual, save?: boolean) => void] => {
...
export async function loadVisual(id: string) {
    ...
    if (visual() == null && storageVisual != null) return setVisual(storageVisual, false);
