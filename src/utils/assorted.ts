...
interface Memo<T extends (...args: any[]) => any, K, M extends MapLike<K, ReturnType<T>>> {
 ...
 */
export function memo<T extends (...args: any[]) => any, K, M extends MapLike<K, ReturnType<T>>>(
    fn: T,
...
export function memoDefault<T extends (...args: any[]) => any>(
    ...
    return memo(fn, (...args) => args[0], new Map<Head<Parameters<T>>, ReturnType<T>>());
}
...
export function openFolder(navigate: Navigator, node: Bookmarks.BookmarkTreeNode | string) {
...
export function openUrlNewTab(url: string | null | undefined, focus: boolean = false) {
    if (url == null) return;
...
export function openUrlClick(url: string | null | undefined, e: OpenUrlModifiers) {
    if (e.ctrlKey) {
        openUrlNewTab(url);
    } else if (e.altKey) {
...
export function makeRejectCancelable<T>(promise: Promise<T>): CancelablePromise<T> {
    let canceled = false;

    const wrappedPromise = new Promise<T>((resolve, reject) => {
        promise.then(value => (canceled ? reject({ isCanceled: true }) : resolve(value)));
...
export function makeSilentCancelable<T>(promise: Promise<T>): CancelablePromise<T | null> {
    let canceled = false;
...
export function applyChanges<T, K extends keyof T>(obj: T, changes: Pick<T, K>): T {
...
export function move<T>(arr: Array<T>, fromIdx: number, toIdx: number): Array<T> {
...
export function zip<A, B>(a: ReadonlyArray<A>, b: ReadonlyArray<B>): ReadonlyArray<[A, B]> {
