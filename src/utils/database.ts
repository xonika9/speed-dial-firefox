...
import { asyncVisitMutate, decodeBlob, encodeBlob, errorSwitch } from './assorted';
...
export async function storageGet<T>(keys: ReadonlyArray<string>): Promise<T | null> {
    const key = storageKey(keys);
    return (await browser.storage.local.get(key))[key] as T | null;
}

export async function storageSet(keys: ReadonlyArray<string>, value: any): Promise<void> {
...
function IdbDatabase(db: IDBDatabase): Database {
    return {
        ...
        set: async (store, key, value) => {
            const transaction = db.transaction([store], 'readwrite');
            return idbRequestToPromise(transaction.objectStore(store).put(value, key)).then();
        },
        remove: async (store, key) => {
            const transaction = db.transaction([store], 'readwrite');
            return idbRequestToPromise(transaction.objectStore(store).delete(key)).then();
        },
        keys: async store => {
            const transaction = db.transaction([store], 'readwrite');
...
const defaultStorageInfoString = 'IndexedDB is unavailable, defaulting to storage.local';
