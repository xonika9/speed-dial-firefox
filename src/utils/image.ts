...
function isBookmarkVisualImage(obj: any): obj is BookmarkVisualImage {
    return obj != null && obj.kind != null && obj.kind === 'image' && isImageType(obj.image);
}
...
export async function scaleDown(image: Image, maxDimSize: number = 512): Promise<Image> {
...
export function isSupportedDomParserType(type: string): type is DOMParserSupportedType {
...
export async function retrieveBlob(url: string | null | undefined): Promise<Blob | null> {
 ...
 */
export async function blobToImage(blob: Blob | null | undefined): Promise<Image | null> {
...
export function retrieveFaviconBlob(url: string): Promise<Blob | null> {
    ...
    return retrieveBlob(`https://frail-turquoise-baboon.faviconkit.com/${domain}/512`);
}

export async function retrieveFaviconBlobSmall(domain?: string): Promise<Blob | null> {
    if (domain == null) return Promise.resolve(null);
    return retrieveBlob(`https://www.google.com/s2/favicons?domain=${domain}&sz=32`);
}

export async function retrieveOpenGraphBlob(url: string, html: Document): Promise<Blob | null> {
    const content = html.querySelector('meta[property="og:image"]')?.getAttribute('content');
    if (content == null) return null;
    return retrieveBlob(convertUrlToAbsolute(url, content));
}

export async function retrieveTwitterBlob(url: string, html: Document): Promise<Blob | null> {
    const content = html.querySelector('meta[name="twitter:image"]')?.getAttribute('content');
    if (content == null) return null;
    return retrieveBlob(convertUrlToAbsolute(url, content));
}

export async function retrieveIconShortcutBlob(url: string, html: Document): Promise<Blob | null> {
    const content = html
...
export async function retrieveAppleIconBlob(url: string, html: Document): Promise<Blob | null> {
    const href = html.querySelector('link[rel="apple-touch-icon"]')?.getAttribute('href');
...
export function largestImage<T extends RasterImage>(images: ReadonlyArray<T>): T | null {
...
export async function retrieveBookmarkImages(url: string): Promise<Array<Promise<Image | null>>> {
    ...
    try {
        ...
        if (!response.ok || contentType == null) return prepareResult([retrieveFaviconBlob(url)]);
        ...
        if (!isSupportedDomParserType(mimeType)) {
            return prepareResult([retrieveFaviconBlob(url), Promise.resolve(mimeImage)]);
        }
...
export async function awaitTabLoad(id: number): Promise<boolean> {
    return new Promise(resolve => {
        const listener = (_tabId: any, changeInfo: Tabs.OnUpdatedChangeInfoType) => {
...
export async function defaultBookmarkVisual(bookmarkId: string): Promise<BookmarkVisual> {
