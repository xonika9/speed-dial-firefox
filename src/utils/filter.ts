...
export function applyFilter<T>(filter?: Filter<T>): (fn: (v: T) => void) => (v: T) => void {
    return fn => v => {
        if (filter == null || filter(v)) fn(v);
    };
}

export function andFilters<T extends Event>(...filters: ReadonlyArray<Filter<T>>): Filter<T> {
    return e => {
        let res = true;
...
export function orFilters<T extends Event>(...filters: ReadonlyArray<Filter<T>>): Filter<T> {
    return e => {
        let res = false;
        for (const filter of filters) {
            res = res || filter(e);
...
export function keyFilter(...keys: ReadonlyArray<string>): Filter<KeyboardEvent> {
...
export function propegationFilter<T extends Event>(sources?: ReadonlyArray<any>): Filter<T> {
    return e => {
