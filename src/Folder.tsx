...
import { CancelablePromise, isChrome, makeSilentCancelable, move } from './utils/assorted';
...
export function FolderState(): FolderState {
    ...
    return {
        ...
        remove: (idx: number) => {
            setState(prev => [...prev.slice(0, idx), ...prev.slice(idx + 1, prev.length)]);
        },
        merge: (nodes: Readonly<Bookmarks.BookmarkTreeNode[]>) => {
            ...
            setState(prev => [...reconcile(nodes, { key: 'id', merge: true })(prev)]);
        },
...
export const FolderSortableItemContext = createSortableItemContext<Bookmarks.BookmarkTreeNode>();
...
export const Folder: Component = () => {
    ...
    let currentChildrenPromise: CancelablePromise<Bookmarks.BookmarkTreeNode[] | null> | undefined;
    createEffect(async () => {
        if (currentChildrenPromise != null) currentChildrenPromise.cancel();
        currentChildrenPromise = makeSilentCancelable(bookmarks.getChildren(params.id));
        ...
        <FolderStateContext.Provider value={state}>
            <Show when={node()}>
                {nnNode => (
                    <div class='header-container'>
                        <Breadcrumb node={nnNode()} onNode={n => navigate(`/folder/${n.id}`)} />
                        <Toolbar node={nnNode()} />
                    </div>
                )}
            </Show>
            <div class='grid-container'>
                ...
                <Show when={nodesLoaded() && state.children().length == 0}>
                    <div class='header-item' style={{ 'font-size': '18px', padding: '4px' }}>
