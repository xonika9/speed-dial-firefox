...
import { assertExhaustive, isValidUrl, openUrlClick, run } from './utils/assorted';
import {
    BiLogosFirefox,
    BiLogosGithub,
    BiRegularMenu,
...
import { propegationFilter, filterPropegation, applyFilter, enterKeyFilter } from './utils/filter';
...
const BookmarkToolbarButtonItem: Component<ToolbarButtonItemProps> = props => {
    ...
    return (
        <Modal show={showNewBookmark()} onClose={() => setShowNewBookmark(false)}>
            ...
            <div class='modal-buttons'>
                <button class={`save ${canNewBookmark() ? '' : 'disabled'}`} onClick={newBookmark}>
                    Create
                </button>
...
const FolderToolbarButtonItem: Component<ToolbarButtonItemProps> = props => {
    ...
    return (
        <Modal show={showNewFolder()} onClose={() => setShowNewFolder(false)}>
            ...
            <div class='modal-separator' />
            <div class='modal-buttons'>
                <button class={`save ${canNewFolder() ? '' : 'disabled'}`} onClick={newFolder}>
                    Create
                </button>
...
const ReloadToolbarButtonItem: Component<ToolbarButtonItemProps> = props => {
    ...
    return (
        <Modal show={showReloadConfirm()} onClose={() => setShowReloadConfim(false)}>
...
const AboutToolbarButtonItem: Component<ToolbarButtonItemProps> = props => {
    ...
    return (
        <Modal show={showAbout()} onClose={() => setShowAbout(false)} closeOnBackgruondClick>
            <div
                style={{
                    display: 'flex',
                    position: 'relative',
...
const CustomizeToolbarButtonItem: Component<ToolbarButtonItemProps> = props => {
    const [showCustomize, setShowCustomize] = createSignal(false);
    props.setOnClick(() => setShowCustomize(true));
    return <CustomizeToolbar show={showCustomize()} onClose={() => setShowCustomize(false)} />;
};
...
function ToolbarButtonWrapper<U extends JSX.Element>(props: ToolbarButtonWrapperProps<U>) {
    const kind = props.kind;
...
export const Toolbar: Component<{ node: Bookmarks.BookmarkTreeNode }> = props => {
    ...
    const toolbarKinds = createMemo(() =>
        props.node.id === rootFolderId ? settings.toolbar.filter(rootFilter) : settings.toolbar,
    );
    ...
    return (
        <div class='header-item dropdown-container' style={{ display: 'flex', gap: '5px' }}>
