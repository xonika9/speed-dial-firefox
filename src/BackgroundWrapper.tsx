...
const BackgroundWrapper: ParentComponent<JSX.HTMLAttributes<HTMLDivElement>> = props => {
    ...
    const style = createMemo(() => (typeof local.style === 'string' ? undefined : local.style));
            ...
            style={{
                'background-image':
                    !settings.useBackgroundColor && background() != null
                        ? `url(${background()})`
                        : '',
                'background-color': settings.useBackgroundColor ? settings.backgroundColor : '',
                ...style(),
            }}
            {...rest}
        >
